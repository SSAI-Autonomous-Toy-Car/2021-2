...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if cfg.STOP_SIGN_DETECTOR:
        ...
        V.add(StopSignDetector(cfg.STOP_SIGN_MIN_SCORE, cfg.STOP_SIGN_SHOW_BOUNDING_BOX), inputs=['cam/image_array', 'user/throttle'], outputs=['user/throttle', 'cam/image_array'])
    
    if cfg.CAR_DETECTOR:
        from donkeycar.parts.car_detector import CarDetector
        V.add(CarDetector(cfg.CAR_SHOW_BOUNDING_BOX, cfg.CAR_THRESHOLD), inputs=['cam/image_array', 'user/throttle'], outputs=['user/throttle', 'cam/image_array'])

    if cfg.LANE_DETECTOR:
        import cv2
        from donkeycar.parts.lane_detection.lane_detector import LaneDetector
        V.add(LaneDetector(cfg.CAR_SHOW_BOUNDING_BOX), inputs=['cam/image_array', 'user/angle', 'user/throttle'], outputs=['user/angle', 'user/throttle'], threaded=True)
    """
    from donkeycar.parts.pixy2.build.python_demos.pixy_detector_add_angle import PixyDetector
    V.add(PixyDetector(True), inputs=['user/angle'],outputs=['user/angle'])
	"""
